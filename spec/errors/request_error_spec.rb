...
describe Yt::Errors::RequestError do
  ...
  describe '#description' do
    context 'given YouTube has error description' do
      let(:body) { {"error"=>"invalid_grant", "error_description"=>"Code was already redeemed."} }
      let(:params) { {response_body: body}.to_json }
      specify 'returns description' do
        expect{error}.to raise_error do |error|
          expect(error.description).to eq "Code was already redeemed."
        end
      end
    end
  end
